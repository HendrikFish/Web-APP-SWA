...
/**
 * Zeigt eine Toast-Nachricht mit Titel und separater Nachricht an
 ...
 */
export function showNotificationToast(title, message, type = 'info', duration = 5000) {
    // Stelle sicher, dass der Toast-Container existiert
    let toastContainer = document.getElementById('toast-container');
    if (!toastContainer) {
        toastContainer = document.createElement('div');
    ...
    // Erstelle das Toast-Element mit Titel und Nachricht
    const toastElement = document.createElement('div');
    toastElement.className = `toast ${config.bgClass} ${config.textClass} border-0`;
    toastElement.id = toastId;
    toastElement.setAttribute('role', 'alert');
    ...
    toastElement.innerHTML = `
        ...
        <div class="toast-body ${config.bgClass} ${config.textClass}" style="opacity: 0.9;">
            ${message}
        </div>
    `;
